...
非同期処理はこの話の中の, 電子レンジでの保温によく似ています. 店員さんは保温する間, 電子レンジの前でただ保温が終わるのを待っているのではなく, 他の作業を行います. 電子レンジが終わったサインを出したタイミングで, 作業を中断して, 電子レンジの中身を取り出す作業に入ります. このように, 自分は別な事をやるけど, 後ろで何らかの仕事をしておいてもらって, 終わったら呼んでもらうというタイプの手続きがプログラミンの中にも出てきます. そのような処理を非同期処理と言います.

以下では, 基本的な非同期処理をするための関数を紹介します.

### setTimeout

ここで setTimeout 関数を説明します. この関数は処理とミリ秒数を引数にとって, 指定されたミリ秒数後に処理を実行します. インタラクティブシェルを使って確認してみましょう.
...
プロミスという名前は, 日本語で約束という意味です. なぜそのような名前がついているのでしょうか? プロミスは, 前節で説明した非同期処理を応用して, 未来のある時に何らかのデータが準備できる事を約束します. 約束されたデータが準備できるとプロミスは解決された, と言われデータを取り出す事が出来ます. 逆に約束が破られる事もあって, その場合はプロミスは拒否されたと言われ, データは取り出せずエラーが返ってきます.
...
Deno のプロミスはこのブザーに似ていて, プロミス自体は値を持っていませんが, プロミスが将来のある時に, データの準備が完了した時に, そのプロミスから値を貰って続きの操作をする事が出来ます.
...
await の使い方は以下のようになります

```
await プロミス
```

このように書くと await プロミスはプロミスが解決するまで処理がそこで止まるようになります. そしてプロミスが解決したタイミングで処理が継続され, プロミスが解決したデータは `await プロミス` という式全体の値として取得する事が出来ます.
...
モジュールの使い方をいくつかみてみましたが, この説ではモジュールを作る方法を説明していきます.

モジュールを作るのは実は難しくありません. 実はモジュールの実体は単なるプログラムのファイルなのです. 通常のプログラムのファイルに export キーワードを使って何がモジュールの機能として使われて良いかを指定してあげれば, それでモジュールが出来ます.
...
その隣に main.js というファイルを作成して次のように入力し, 保存してください.
...
これで, hello.js というモジュールを作ったことになります.

hello.js の中で `export function hello() {}` と書きましたが, これはこの hello 関数をモジュールの部品として外から使う事を許可する命令です.

main.js の中の `import { hello } from './hello.js'` ですが, これは hello.js から hello 関数を利用するという指示です. この指示によって, main.js の中で hello.js の中の hello 関数が使えるようになりました.

### 6-3 モジュールを使ってサーバーを作ってみる

最後にモジュールの機能を使って Deno で HTTP サーバーを作ってみましょう. その前にサーバーとは何かを簡単に説明します.

普段私たちはインターネットで色々なサイトをみていますが, それらのサイトは全てサーバーから送られてきています. コンピュータはサーバーに URL を問い合わせて, その URL に対応するファイルを受け取ってそれをインターネット上のサイトとして表示しています. つまりサーバーとは URL のリクエストに対して, 対応するファイルを返すプログラムのことです.

```
for await (req of Deno.serve('')) {
}
```
