...
以上のステップを終えると次のお客さんの会計処理が始まる. これは同期処理です. なぜなら1つ1つの処理が全て順番に行われているからです. ここで, 弁当を温めるパターンが含まれていた場合を考えてみましょう.
...
という処理が差し込まれます. これが非同期処理です. より細かくいうと, 電子レンジで保温する処理が非同期処理です. なぜなら店員さんは電子レンジでの保温中に, その完了を待たずに別の作業をしているからです.

このようにある処理を別の何かにお願いして, 自分は別のことを先に処理するというところが非同期処理の本質です.
...
1がすぐに表示されて, 2が少し後で表示されたはずです. なぜ2が先に表示されないのでしょうか? これが上で説明した非同期処理の話につながってきます. 上のプログラムでは `function () { console.log(2) }` という処理を1秒後(1000ミリ秒後)に実行してほしいという, 非同期処理の登録を行いました. その時点では登録だけが行われ, 処理はそのまま進んで `console.log(1)` が実行され, 1が表示されます. その1秒後に非同期処理として登録された処理が実行され, 2 が表示されたのです.

### まとめ

Deno の処理には同期処理と非同期処理の違いがあることを説明しました. 通常の処理は同期処理と呼ばれ前から順番に実行されます. 非同期処理として登録された処理は, なんらかのタイミングで後のタイミングで実行されます. 非同期処理を登録してもそこで処理は止まらずに, 後ろに同期処理があれば, そちらの同期処理が優先的に実行されます. 非同期処理は適切なタイミングになった時に実行されます.

setTimeout という関数が非同期処理を実行する基本的な関数で, この関数を使うことで一定時間後に, 処理を非同期に実行する事が出来ました.

## 5-2 未来のデータを扱う - プロミス

非同期処理を応用したデータ型でプロミスというものがあります. この節ではプロミスがどういうものかを説明していきます.

プロミスは名前が示すように未来のデータの約束のようなもののイメージです. 例えていうと, フードコートなどで商品を購入した時に渡されるブザーに似ています. そのブザーを持ってあなたは自分の席を選んだり, 人と喋ったり好きな事が出来ますが, ブザーが鳴ったらその商品を取りにいきます. ブザー自体は商品ではありませんが, ブザーが鳴ることで, それを商品と交換する事ができるようになります.

Deno のプロミスも同様です. プロミス自体は値を持っていませんが, プロミスが将来のある時に, データの準備が完了した時に, そのプロミスから値を貰って続きの操作をする事が出来ます.
...
Promise {} に続いて `Response { status, url, statusText, type, bodyUsed, trailer, headers, body, redirected }` という行が表示されました. これは fetch 関数が返す Response オブジェクトというものになっています. この Response オブジェクトの中に https://example.com に関するいろいろな情報が入っています. Response オブジェクトの中のいくつかのプロパティを表示して調べてみましょう
