...
文字列型には様々なメソッドと呼ばれるものがあります. ここではそのいくつかを紹介します.

##### replace

replace メソッドは文字列の中に現れる特定の文字列を, 特定の文字列で置き換えます. replace メソッドは次の書式で実行します.

```
文字列.replace('置き換える元の文字列', '置き換えた後の文字列')
```

実際に試してみましょう.

```
> 'Hello John'.replace('John', 'Taro')
Hello Taro
```

上の例では 'Hello John' に対して replace メソッドを使っています. 置き換える文字列を 'John', 置き換える結果を 'Taro' とした結果, 'Hello Taro' という文字列に置き換わりました.

##### includes

includes メソッドは文字列の中に特定の文字列パターンが含まれるかを判定します.

```
文字列.includes('判定する文字列')
```

実際に試してみましょう.

```
> 'abcdefghi'.includes('def')
true
> 'abcdefghi'.includes('xyz')
false
```

これは 'abcdefghi' という文字列の中に 'def' が含まれている (true) ことと, 'xyz' が含まれていない (false) ことを表しています.
